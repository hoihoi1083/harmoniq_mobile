...
function calculateFortuneScore(birthPillars, datePillars) {
  ...
  const combiningPairs = [[0,1], [2,11], [3,10], [4,9], [5,8], [6,7]];
  for (const [a, b] of combiningPairs) {
    if ((birthBranchIndex === a && dateBranchIndex === b) ||
  ...
  const dayFactor = (datePillars.day.charCodeAt(0) + datePillars.day.charCodeAt(1)) % 20 - 10;
  score += dayFactor;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Generate details
    const details = generateFortuneDetails(fortune.level, score, birthPillars, datePillars);

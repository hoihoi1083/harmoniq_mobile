...
export default function BaziDetailedChart({ baziData }) {
	try {
		...
		console.log("relationships.stemCombinations:", relationships?.stemCombinations);
		console.log("relationships.branchRelationships:", relationships?.branchRelationships);
		console.log("relationships.threeHarmonies:", relationships?.threeHarmonies);
		...
		const getChartFeatures = () => {
			...
			// 天干關係
			if (relationships?.stemCombinations && relationships.stemCombinations.length > 0) {
				relationships.stemCombinations.forEach(rel => {
					features.push(`${rel.stem1}${rel.stem2}${rel.relationship}`);
				});
			}
			
			// 地支關係
			if (relationships?.branchRelationships && relationships.branchRelationships.length > 0) {
				relationships.branchRelationships.forEach(rel => {
					features.push(`${rel.branch1}${rel.branch2}${rel.relationship}`);
				});
			}
			
			// 三合局
			if (relationships?.threeHarmonies && relationships.threeHarmonies.length > 0) {
		...
		<div className="w-full min-h-screen p-4 pt-6 bg-gradient-to-b from-blue-50 to-white">
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2">
							<span className="px-2 py-1 text-xs font-semibold text-orange-700 bg-orange-100 rounded">農曆</span>
							<span className="text-sm text-gray-700">
								{lunarDate?.formatted || baziData?.lunarDateString || "農曆資料"}
							</span>
						</div>
						<div className="flex items-center gap-2">
							<span className="px-2 py-1 text-xs font-semibold text-orange-700 bg-orange-100 rounded">陽曆</span>
					...
					{pillarLabels.map((label, index) => {
						...
						return (
							<div key={key} className="text-center">
								...
								<div className={`p-2 rounded-lg ${isDayMaster ? "border-2 border-yellow-400 bg-yellow-50" : "bg-gray-50"}`}>
									<div className="text-xs text-gray-600 mb-1">{pillar?.stemTenGod || "-"}</div>
									...
									{isDayMaster && <div className="text-xs text-yellow-600 mt-1">日主</div>}
								</div>
							</div>
						);
					})}
				...
				<table className="w-full border-collapse table-fixed">
					...
					<tbody>
						...
						<tr>
							...
							{(detailedData?.stemTenGods || []).map((god, i) => (
								<td
									key={i}
						...
						{[0, 1, 2].map((rowIndex) => (
							<tr key={`hidden-${rowIndex}`}>
								...
								{detailedData.hiddenStems.map(
									(pillarHidden, i) => {
										const hiddenData = pillarHidden[rowIndex];
										return (
											<td
												key={i}
												className="px-2 py-2 text-sm font-semibold text-center border border-gray-300"
												style={{ 
													color: hiddenData ? getElementColor(hiddenData[1]) : '#999'
												}}
											>
												{hiddenData ? hiddenData[0] : "-"}
											</td>
										);
									}
								)}
						...
						<tr>
							...
							{(detailedData?.nayin || ["-", "-", "-", "-"]).map((nayin, i) => (
								<td
									key={i}
			...
			<div className="mt-4">
				<div className="flex items-center gap-2 mb-2">
					...
					<h3 className="text-lg font-bold text-gray-800">命盤特徵</h3>
				</div>
				
				<div className="space-y-2">
					{/* 天干關係 */}
					{relationships?.stemCombinations && relationships.stemCombinations.length > 0 && (() => {
						// Deduplicate by creating unique keys
						const seen = new Set();
						const unique = relationships.stemCombinations.filter(rel => {
							const key = [rel.stem1, rel.stem2].sort().join('');
						...
						return unique.length > 0 ? (
							<div className="p-3 bg-white rounded-lg shadow-sm">
								<div className="mb-1 text-xs font-semibold text-olive-600">天干</div>
								<div className="text-sm text-gray-700">
									{unique.map((rel, i) => (
										<span key={i}>
											{rel.stem1}{rel.stem2}{rel.type || rel.relationship || "合"}
											{i < unique.length - 1 && "；"}
										</span>
									))}
								</div>
					...
					{/* 地支關係 */}
					{relationships?.branchRelationships && relationships.branchRelationships.length > 0 && (() => {
						// Deduplicate by creating unique keys
						const seen = new Set();
						const unique = relationships.branchRelationships.filter(rel => {
							const key = [rel.branch1, rel.branch2].sort().join('') + rel.relationship;
							if (seen.has(key)) return false;
							seen.add(key);
							return true;
						});
						return unique.length > 0 ? (
							<div className="p-3 bg-white rounded-lg shadow-sm">
								<div className="mb-1 text-xs font-semibold text-olive-600">地支</div>
								<div className="text-sm text-gray-700">
									{unique.map((rel, i) => (
										<span key={i}>
					...
					{relationships?.threeHarmonies && relationships.threeHarmonies.length > 0 && (
						<div className="p-3 bg-white rounded-lg shadow-sm">
							<div className="mb-1 text-xs font-semibold text-olive-600">整柱</div>
							<div className="text-sm text-gray-700">
								{relationships.threeHarmonies.map((harmony, i) => (
									<span key={i}>
										{harmony.branches ? harmony.branches.join("") : ""}{harmony.type || ""}
										{i < relationships.threeHarmonies.length - 1 && "；"}
									</span>
								))}
							</div>
						</div>
		...
		return (
			<div className="w-full min-h-screen p-4 pt-6 bg-gradient-to-b from-blue-50 to-white">
				<div className="p-4 bg-red-50 border border-red-200 rounded-lg">
					<h3 className="text-lg font-bold text-red-700 mb-2">載入錯誤</h3>
					<p className="text-sm text-red-600">無法顯示八字詳細排盤，請重試。</p>

...
import CoupleReportDoc from "@/models/CoupleReportDoc"; // 🎯 新增：合盤報告模型
...
function cleanMarkdownFormatting(text) {
	...
	// 移除 ## 標題標記，保留內容
	let cleaned = text.replace(/^#{1,6}\s+/gm, ""); // Remove # headers at start of line
...
// 🧹 Clean up lunar calendar terms from AI responses
function cleanLunarCalendarTerms(text) {
	if (!text || typeof text !== "string") return text;

	return (
		text
			// Remove 農曆 prefix from months
			.replace(/農曆([一二三四五六七八九十]+月)/g, "$1")
			.replace(/農曆([0-9]+月)/g, "$1")
			.replace(
			...
			.replace(/農曆(廿[一二三四五六七八九十]+)/g, "每月$1")
			// Replace specific problematic phrases
			.replace(/農曆九月出生/g, "9月出生")
			.replace(/農曆十月出生/g, "10月出生")
			.replace(/農曆八月出生/g, "8月出生")
			.replace(/農曆七月出生/g, "7月出生")
			// Clean up day references
			.replace(/農曆初七/g, "每月7日")
			.replace(/農曆十五/g, "每月15日")
			.replace(/農曆十九/g, "每月19日")
			// Remove any remaining 農曆 references
			.replace(/農曆/g, "")
	);
}

// 🎂 檢查用戶是否有已保存的生日並生成相應消息
async function checkSavedBirthdayAndGenerateMessage(
	userEmail,
	userId,
	topic,
	aiResponse = null,
	...
	try {
		...
		if (existingBirthdayData && existingBirthdayData.userBirthday) {
			// 格式化已保存的生日
			const savedDate = new Date(existingBirthdayData.userBirthday);
			...
			console.log("🔍 locale === 'zh-CN':", locale === "zh-CN");
			console.log("🔍 locale type:", typeof locale);
			console.log("🔍 locale value strict:", JSON.stringify(locale));

			// Localized text
			const texts =
				locale === "zh-CN"
					? {
							fenglingFound: "小铃发现你之前提供过生日资料呢！📅",
...
function parseCouplesBirthdays(message) {
	...
	const couplePatterns = [
		// 我XXX，她XXX (male user, female partner) - 如果說"她"，用戶應該是男性
		{
			pattern:
		...
		// 我XXX，他XXX (female user, male partner) - 如果說"他"，用戶應該是女性
		{
			pattern:
		...
		// 我的生日是XXX，她的生日是XXX (male user, female partner)
		{
			pattern:
...
// 🔧 格式化合盤分析回應 - 將結構化對象轉換為字符串
function formatCoupleAnalysisResponse(analysisResult, locale = "zh-TW") {
...
告訴小鈴你的生日，我可以幫你看看${topicText}方面的運勢哦！
...
小鈴會先給你一個簡單的分析，如果你覺得有幫助，還可以做更詳細的完整報告哦～💕${rateLimitInfo}`;
	}
...
- 命理分析（八字解讀、流年運勢）`;
		...
		const zodiacYearContext = `\n當前生肖年：2025年是乙巳蛇年（Snake Year），不是馬年（Horse Year）`;

		switch (redirectLevel) {
			case "firm":
				return `用戶問題：${question}

當前日期：${currentDateStr}（僅作為內部參考，不需要在回應中提及具體日期）${zodiacYearContext}

你是小鈴，用戶已經連續問了多個與風水命理無關的問題。需要友善但堅定地引導用戶使用你的專業服務。

重要指示：${languageInstruction}，不要包含字數統計標記。如果需要提及時間，請確保是當前的2025年，避免提及2024年或過去的時間。只在真正需要時間背景的情況下才提及日期。2025年是蛇年，不要誤稱為馬年。
...
你是小鈴，用戶問了一些與風水命理無關的問題。需要在回答的同時積極推薦你的專業服務。
...
你是友善的小鈴，用戶問了一個與風水命理無關的問題。請給用戶實用回答，然後積極引導他們了解你的專業服務。重要指示：${languageInstruction}，不要包含字數統計標記。如果需要提及時間，請確保是當前的2025年，避免提及2024年或過去的時間。只在真正需要時間背景的情況下才提及日期，不要強制在每個回應中都包含。2025年是蛇年，不要誤稱為馬年。
...
請以小鈴的身份，針對這個具體的工作問題，提供自然對話式的回應。
...
想不想知道更多升職的小魔法呀？`;
				...
				response = `了解到你在工作上遇到挑戰，讓小鈴來幫你分析一下！💪
...
請以小鈴的身份，針對這個具體的財運問題，提供自然對話式的回應。
...
請以小鈴的身份，針對這個具體的健康問題，提供自然對話式的回應。
...
請以小鈴的身份，針對這個具體的命理問題，提供自然對話式的回應。
...
小鈴會先給你一個簡單的命理分析，如果你覺得有幫助，還可以做更詳細的完整八字報告哦～🔮💕`
			);
		} catch (error) {
			console.error("AI測算命理回應失敗:", error);
			return `哇～你想了解命理呀！小鈴最喜歡幫人解析運勢啦！🔮✨
...
小鈴會先給你一個簡單的命理分析，如果你覺得有幫助，還可以做更詳細的完整八字報告哦～🔮💕`;
	...
	// 		const fengShuiGreeting = `哇～你想了解子女呀！小鈴最喜歡幫人解決問題啦！✨
	...
	// 		const fengShuiGreeting = `哇～你想了解因緣呀！小鈴最喜歡幫人解決問題啦！✨
...
請以小鈴的身份，針對這個具體的${topic}問題，提供自然對話式的回應。
...
小鈴會先給你一個簡單的分析，如果你覺得有幫助，還可以做更詳細的完整報告哦～💕`
			...
			// 備用模板回應
			return `哇～你想了解${analysis.detectedTopic}呀！小鈴最喜歡幫人解決問題啦！✨

每個人的運勢都不一樣呢，就像每個人的生日不一樣一樣！

告訴小鈴你的生日，我可以幫你看看${analysis.detectedTopic}方面的運勢哦！
...
重要指示：1. ${languageInstruction} 2. 在回應中所有日期和月份都必須使用新歷（西曆/公曆），例如1月、2月、3月等，不要使用農歷 3. 不要在回應中包含字數統計標記 4. 如果需要提及時間，請確保是當前的2025年，避免提及2024年或過去的時間 5. 只在真正需要時間背景的情況下才提及日期，不要強制在每個回應中都包含 6. 2025年是蛇年，不要誤稱為馬年或其他生肖年。
...
3. 八字分析、流年運勢、生肖運勢（如馬年、蛇年影響）、命盤、紫微斗數 → 歸類為「命理」
4. 如果用戶輸入很簡單（如只是"感情"、"財運"、"命理"），specificProblem應該保持簡潔，如"一般感情諮詢"或"一般命理諮詢"
5. 如果問題不在我們服務範圍內（如天氣、科技、日常閒聊等），請設定 isWithinScope 為 false，並在 aiResponse 中提供禮貌友善的回應，自然地確認用戶的話題
6. 對於數字或選擇類輸入（如"1"、"2"），通常表示用戶在回應選項，設為不在範圍內
7. aiResponse 應該禮貌回應用戶，不要直接說"不在服務範圍"，而是自然地承接話題
8. 在 serviceRecommendation 中智能引導到相關的風水服務
...
我們提供的服務領域：
- 感情：戀愛、分手、復合、合盤、桃花運、婚姻、感情問題、約會、結婚
...
- 八字分析、流年運勢、生肖運勢（如馬年、蛇年影響）、命盤、紫微斗數 → 歸類為「命理」`;
...
4. 緊急情況優先提供立即可行的建議
5. 考慮用戶的情緒需求，不只是技術建議
6. **aiResponse ${languageInstruction}，語言風格要溫暖親切**
7. **對於不在服務範圍內的問題（其他話題），請提供實用的回答建議，然後自然地引導到風水命理服務**
8. **不要推薦不存在的服務，只能推薦以下實際提供的服務：感情運勢分析、工作事業分析、財運分析、健康運勢、命理分析**
9. **serviceRecommendation 只能從以上實際服務中選擇，不要創造新的服務名稱**
10. **八字、流年、生肖運勢問題應歸類為「命理」**
...
重要指示：1. ${languageInstruction} 2. 所有日期和月份都必須使用新歷（西曆/公曆），不要使用農歷。例如：1月、2月、3月等，避免使用農曆1月、農曆8月等表達方式 3. 🚫 嚴格禁止任何農曆用詞：農曆九月、農曆十月、農曆初七、農曆十五等 4. ✅ 正確用法：9月出生、10月出生、每月7日、15日等 5. 不要在回應中包含字數統計如（72字）等標記 6. 保持小鈴親切可愛的語氣風格 7. 請確保所有時間相關的回應都基於當前日期${currentDateStr}，避免提及2024年或過去的時間 8. 2025年是蛇年，不要誤稱為馬年或其他生肖年。`,
			...
			return cleanLunarCalendarTerms(
...
你對哪一種有興趣？`
			...
			case "感情":
				return this.generateEmotionFlow(
					analysis,
					originalMessage,
					this.locale
				);
	...
	generateEmotionFlow(analysis, originalMessage, locale = "zh-TW") {
...
根据你的状态，小铃会为你量身打造最适合的感情指导～💕`
					: `哇～分手真的很難過呢... 小鈴給你一個大大的抱抱！🤗💕
			...
			const aiAnswer = await this.callDeepSeekAPI([
				{
					role: "system",
					content:
						"你是親切可愛的小鈴，善於先回答用戶問題再自然地介紹自己的專業服務。你只提供風水命理相關服務，不要推薦不存在的服務。\n\n⚠️ 重要指示：\n1. ${languageInstruction}\n2. 所有日期和月份都必須嚴格使用西曆（公曆），絕對不可使用農曆用詞\n3. 🚫 禁止用詞：「農曆十月」「陰曆八月」「農曆四月」等\n4. ✅ 正確用法：「10月出生者」「十月份」「8月」等\n5. 當前是2025年11月10日，當前生肖年是乙巳蛇年（Snake Year），不是馬年（Horse Year）\n6. 請確保回應基於2025年，不可提及2024年\n7. 用戶提供的生日是西曆日期，請據此分析\n8. 不要在回應中包含字數統計標記\n9. 保持小鈴親切可愛的語氣風格\n10. 2025年是蛇年，不要誤稱為馬年或其他生肖年",
				},
				{
					role: "user",
...
請以小鈴的身份，按照以下格式測算詳細的八字分析報告：

🔮 小鈴看了你的八字，發現你有很特別的${displayTopic}潛質呢！💼
...
💖 哈囉親愛的[五行]命小夥伴！讓小鈴為你解鎖${new Date().getFullYear()}年的${displayTopic}密碼～  
...
3. ✅ 正確示例：「10月出生者」「十月份」「8月」
4. ❌ 錯誤示例：「農曆十月出生者」「陰曆八月」
5. 當前是2025年10月，分析需基於2025年時間軸
6. 用戶生日是西曆格式，請據此進行命理分析
...
[五行]命寶寶記得多用「[五行相生原理]」原理，[具體建議]能讓你[效果]！✨ 有問題隨時喚醒小鈴喔～
...
8. 特別注意：用戶的生日是西曆日期，不是農曆，請據此分析命理特質`;
		...
		try {
			...
			if (
				...
				return cleanLunarCalendarTerms(aiResponse);
			} else {
				throw new Error("AI回應格式異常");
			}
		} catch (error) {
			console.error("❌ 生成詳細八字分析失敗:", error);
			return `🔮 小鈴看了你的八字 ${baziString}，很想為你分析${displayTopic}呢！
	...
	try {
		...
		if (userBirthday && gender && !message?.trim()) {
			...
			if (parsedDate) {
				...
				userIntent.birthdayConfirmed = true; // 🎂 標記生日已確認
		...
		const textChoicePattern =
			/^(個人分析|個人感情分析|合盤分析|合盤配對分析|個人|合盤|配對)$/i;
...
請回覆「1」或「2」～`;
		...
		// 🔧 容錯條件：如果偵測到雙方生日，且符合以下任一條件，則執行合盤分析：
		// 1. relationshipAnalysisType 明確為 "couple" (標準流程)
		// 2. primaryConcern 是 "感情" (用戶選擇感情但尚未明確選擇合盤/個人)
		const shouldProcessCouple =
			couplesBirthdayData &&
			(userIntent?.relationshipAnalysisType === "couple" ||
				userIntent?.primaryConcern === "感情");

		console.log("   shouldProcessCouple:", shouldProcessCouple);

		if (shouldProcessCouple) {
			// 🎯 處理合盤分析的雙方生日輸入
			console.log("🎯 檢測到合盤分析中的雙方生日:", couplesBirthdayData);

			// 🔧 如果 relationshipAnalysisType 未設定，在這裡設定為 "couple"
			if (!userIntent) {
				userIntent = new SmartUserIntent({
					sessionId: sessionId,
					userEmail: userEmail,
				...
				userIntent.relationshipAnalysisType = "couple";
				console.log(
					"🔧 自動設定 relationshipAnalysisType = couple (因檢測到雙方生日)"
				);
			}
			...
			try {
				...
				if (
					...
					response = formatCoupleAnalysisResponse(
			...
			// 🎯 一旦合盤分析已生成，立即回傳結果，避免後續分支覆蓋 response
			return NextResponse.json({
				response,
				analysis,
				systemType: "smart-chat2",
			...
			try {
				...
				if (!userIntent) {
					...
					// 🎂 更新現有的用戶意圖 - 保持原始主題，不覆蓋
					// 如果用戶已經有 primaryConcern，保持不變（用戶可能在輸入新生日）
					if (!userIntent.primaryConcern) {
						userIntent.primaryConcern = topicAndBirthdayData.topic;
					}
					// 保持原始問題，不用生日日期覆蓋
					if (!userIntent.originalUserMessage) {
						userIntent.originalUserMessage =
							topicAndBirthdayData.originalMessage;
					}
					userIntent.conversationState = "asking_detailed_report";
...
讓小鈴為你做詳細的${topicAndBirthdayData.topic}分析～
			...
			if (hasBreakupOptions) {
				...
				switch (breakupChoice) {
					case "A":
						...
						response =
							locale === "zh-CN"
								? `小铃完全理解你现在的心情，刚分手真的很痛苦... 

给自己一些时间疗伤是很重要的，不要急着压抑情绪哦！

**🌸 现阶段最重要的建议：
**情感修复期** - 允许自己悲伤，但不要沉溺太久
**能量清理** - 整理房间，特别是感情角落（西南方）
**自我照顾** - 多接触阳光，避免长期待在阴暗空间
...
要记住，每一次结束都是为了更好的开始！小铃会陪着你走过这段路的 🤗
...
要記住，每一次結束都是為了更好的開始！小鈴會陪著你走過這段路的 🤗
...
你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
					...
					case "B":
						...
						response =
							locale === "zh-CN"
								? `🌟 太好了！能够准备好重新开始，代表你已经成长了很多呢！

现在是最适合重新出发的时候，小铃来帮你做好准备！
...
你想要哪种分析呢？回复「个人分析」或「合盤分析」即可～`
								: `🌟 太好了！能夠準備好重新開始，代表你已經成長了很多呢！

現在是最適合重新出發的時候，小鈴來幫你做好準備！
...
你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
					...
					case "C":
						...
						response =
							locale === "zh-CN"
								? `💭 想复合的心情小铃很理解，但这确实需要谨慎考虑呢...

让我们一起分析一下复合的可能性和最佳策略！
...
你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
					...
					case "D":
						...
						response =
							locale === "zh-CN"
								? `✨ 哇！你的心境调整得真好，能够放下过去迎向未来，这份勇气很棒呢！

现在就是你桃花大开的最佳时机，小铃来帮你吸引理想对象！
...
你想要哪种分析呢？回复「个人分析」或「合盤分析」即可～`
								: `✨ 哇！你的心境調整得真好，能夠放下過去迎向未來，這份勇氣很棒呢！

現在就是你桃花大開的最佳時機，小鈴來幫你吸引理想對象！
...
直接回覆字母 A、B、C 或 D 即可！`;
		...
		if (
			...
			if (!existingSavedBirthday) {
				console.log("⚠️ 用戶選擇了 1 或 2，但沒有找到已保存的生日");
				// 如果沒有已保存的生日，要求輸入
				response = `抱歉，小鈴沒有找到你之前保存的生日記錄呢。
...
小鈴會先給你一個簡單的分析，如果你覺得有幫助，還可以做更詳細的完整報告哦～💕`;
				...
				analysis = {
					detectedTopic: userIntent.primaryConcern || "其他",
					isWithinScope: true,
					confidence: 0.95,
					specificProblem: "等待輸入生日",
				};
				...
				// 🎂 將已保存的生日更新到當前 userIntent
				userIntent.userBirthday = existingSavedBirthday.userBirthday;
				...
				const concern = userIntent.primaryConcern || "綜合運勢";
...
請回覆「1」或「2」選擇你想要的報告～`; // 更新狀態
				...
				await DailyAnalysisRateLimit.recordAnalysis(
					...
					"individual",
					concern,
					userIntent.originalUserMessage || specificQuestion
				);

				analysis = {
					detectedTopic: concern,
					isWithinScope: true,
					confidence: 0.95,
					specificProblem: specificQuestion,
				};
			} else {
				// 用戶選擇輸入新生日
				console.log("🎂 用戶選擇輸入新生日");
...
小铃会先给你一个简单的分析，然后你可以选择是否需要更详细的报告！✨`
						: `好的！請告訴小鈴你的新生日～
...
小鈴會先給你一個簡單的分析，然後你可以選擇是否需要更詳細的報告！✨`;

				// 保持在 birthday_collection 狀態，但清除已保存的生日標記，允許輸入新的
				userIntent.conversationState = "birthday_collection";
				// 不清除 userBirthday，這樣如果用戶不輸入新的，仍可使用舊的
				await userIntent.save();
...
(請等待系統為你測算詳細的${userIntent.primaryConcern}分析報告)`;
			...
			if (reportType === "detailed_concern") {
				...
				if (
					...
					// 合盤分析詳細報告 - 先生成並保存到數據庫
					try {
						// 🔮 生成合盤分析數據 (使用現有的 EnhancedInitialAnalysis 邏輯)
						...
						// 🎯 創建並保存詳細合盤報告到數據庫 (複製自 Smart-Chat 邏輯)
...
正在準備你的專屬報告...`;
			...
			if (!choice) {
				...
				if (/^(個人分析|個人感情分析|個人)$/i.test(textMessage)) {
					...
					/^(合盤分析|合盤配對分析|合盤|配對)$/i.test(textMessage)
				) {
					choice = "2";
				}
			}
...
請提供：出生年月日（例如：1990年5月15日）`;
					...
					// 🔧 完全複製 smart-chat 合盤分析回應
					response =
						locale === "zh-CN"
							? `💕 好的！为了进行准确的合盤分析，我需要你们双方的生日资料。

请先提供**你的生日**（年月日），例如：1995年3月15日

💡 小贴士：你也可以一次提供双方生日，例如：「我1995/3/15，她1996/8/20」`
							: `💕 好的！為了進行準確的合盤分析，我需要你們雙方的生日資料。
...
正在準備你的專屬報告...`;
					...
					response =
						locale === "zh-CN"
							? `好的！告诉小铃你的生日，我来为你做${userIntent.primaryConcern}分析：
...
小铃会先给你一个简单的分析，如果你觉得有帮助，还可以做更详细的完整报告哦～💕`
							: `好的！告訴小鈴你的生日，我來為你做${userIntent.primaryConcern}分析：
...
小鈴會先給你一個簡單的分析，如果你覺得有幫助，還可以做更詳細的完整報告哦～💕`;
			...
			if (
				...
				if (
					...
					// 🔧 簡化邏輯：默認為個人分析，只有在明確有伴侶生日時才做合盤分析
					if (
						concernName === "感情" &&
						userIntent.relationshipAnalysisType === "couple" &&
						userIntent.partnerBirthday &&
						...
						// 有完整雙方生日的合盤分析 - 仍使用合盤分析
						shouldTriggerModal = true;
						userIntent.conversationState =
							"collecting_payment_info";
						response = `💕 太好了！準備為你們製作專屬的合盤分析報告！
...
請填寫付款資訊，我們立即開始製作你們的專屬報告～`;
						...
						analysis = {
							...
							confidence: 0.95,
							reportChoice: true,
							paymentType: "couple", // 🔥 修改：合盤分析使用 couple payment API
						};

						console.log("🔒 合盤報告選擇時保護原始問題:");
						console.log(
							"   originalSpecificProblem:",
							userIntent.originalSpecificProblem
						);
...
請填寫個人資料，我們開始製作你的專屬佈局方案～`;
			...
			if (detectedTopic && coreTopics.includes(detectedTopic)) {
				...
				// 🌸 感情主題特殊處理 - 需要先選擇個人分析或合盤分析
				if (detectedTopic === "感情") {
					if (aiResponse && aiResponse.trim()) {
						combinedResponse = `${aiResponse}\n\n💫 已為你切換到感情分析！\n\n為了提供最適合的分析，請選擇：\n\n1️⃣ 個人感情分析\n- 分析你的桃花運勢和感情特質\n- 提供感情運勢建議和風水調整\n- 適合單身或想了解個人感情運的朋友\n\n2️⃣ 合盤配對分析\n- 分析你和伴侶的八字契合度\n- 提供雙方感情走向和發展建議\n- 適合想了解感情配對度的情侶\n\n你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
					} else {
						combinedResponse = `💫 已為你切換到感情分析！\n\n為了提供最適合的分析，請選擇：\n\n1️⃣ 個人感情分析\n- 分析你的桃花運勢和感情特質\n- 提供感情運勢建議和風水調整\n- 適合單身或想了解個人感情運的朋友\n\n2️⃣ 合盤配對分析\n- 分析你和伴侶的八字契合度\n- 提供雙方感情走向和發展建議\n- 適合想了解感情配對度的情侶\n\n你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
					}
				} else {
					// 🎂 其他主題 - 檢查是否有已保存的生日
					console.log(
				...
				userIntent.specificQuestion = specificProblem; // 更新資料庫狀態
				await userIntent.save();
				...
				// 🎯 添加額度信息到回應訊息
				const quotaMessage = rateLimitInfo
				...
				return NextResponse.json({
					...
					needsBirthday: detectedTopic !== "感情", // 感情主題先不要birthday，先選擇類型
					...
					if (
						...
						if (aiTopicAnalysis.detectedTopic === "感情") {
							...
							if (
								...
								// 如果有已保存的生日，birthdayCheck.message 已經包含了 AI 分析
								// 如果沒有已保存的生日，需要手動組合
								if (birthdayCheck.hasSavedBirthday) {
									combinedResponse = `💫 已為你切換到${aiTopicAnalysis.detectedTopic}分析！\n\n${birthdayCheck.message}`;
								} else {
									combinedResponse = `${aiTopicAnalysis.aiResponse}\n\n💫 已為你切換到${aiTopicAnalysis.detectedTopic}分析！\n\n${birthdayCheck.message}`;
								}
							} else {
								combinedResponse = `💫 已為你切換到${aiTopicAnalysis.detectedTopic}分析！\n\n${birthdayCheck.message}`;
							}
						...
						userIntent.specificQuestion =
							aiTopicAnalysis.specificProblem; // 更新資料庫狀態
						await userIntent.save();
						...
						return NextResponse.json({
							...
							conversationState:
								aiTopicAnalysis.detectedTopic === "感情"
									? "asking_relationship_type"
									: "birthday_collection",
							systemType: "smart-chat2",
							timestamp: new Date().toISOString(),
							...
							needsBirthday:
								aiTopicAnalysis.detectedTopic !== "感情", // 感情主題先不要birthday，先選擇類型
							specificQuestion:
								aiTopicAnalysis.specificProblem ||
...
💡 小貼士：你也可以一次提供雙方生日，例如：「我1995/3/15，她1996/8/20」`;
							console.log("⚠️ 合盤分析 - 缺少生日資料，重新收集");
						}
					} else {
						// 個人分析或其他分析
						shouldTriggerModal = true;
...
正在準備你的專屬報告...`;
					...
					analysis = {
						...
						paymentType:
							userIntent.relationshipAnalysisType === "couple"
								? "couple"
								: "fortune", // 🔥 新增：合盤使用couple，個人使用fortune API
					};
				} else if (
					reportChoice === "2" ||
					reportChoice.includes("綜合") ||
...
這樣我就能為你做精準的八字分析啦！✨`;
			...
			if (baziTopicResult) {
				...
				if (baziTopicResult.analysisType === "bazi_topic_analysis") {
					...
					const serviceMenuResponse = `🔮 小鈴看到你的八字了：${baziTopicResult.baziString}
...
你想了解哪個方面呢？直接告訴我「我想了解感情」或「幫我看工作」即可～✨`;
			...
			try {
				...
				// 🔧 BREAKUP CHOICE INTERCEPTOR - Must return directly to skip all other processing
				if (isBreakupChoice && userIntent?.primaryConcern === "感情") {
					...
					const hasBreakupMenu = chatHistory?.messages?.some(
						(msg) =>
							msg.role === "assistant" &&
							(msg.content.includes("A. 剛分手") ||
								msg.content.includes("A. 刚分手"))
					);

					if (hasBreakupMenu) {
						console.log(
							"✅ 發現分手菜單，直接處理A/B/C/D選擇並返回"
						);
						let breakupChoice = message
							.trim()
							.toUpperCase()
							.charAt(0);
						let breakupResponse = "";
...
你想要哪種分析呢？回覆「個人分析」或「合盤分析」即可～`;
						...
						if (breakupResponse) {
							...
							// ✅ 直接返回response，跳過所有其他處理
							return NextResponse.json({
								response: breakupResponse,
								conversationState: "breakup_advice",
								systemType: "smart-chat2",
								timestamp: new Date().toISOString(),
								concern: "感情",
								emotion: "supportive",
								...
								reportUrl: null,
								needsBirthday: false,
								specificQuestion: `分手狀態選擇: ${breakupChoice}`,
								isCoupleAnalysis: false,
							});
						}
					}
...
請以小鈴的身份，用可愛親切的語氣為用戶分析八字。八字資料：
- 八字：${enhancedResult.baziData?.baziString || ""}
- 主要分析領域：${enhancedResult.specificProblem || "八字分析"}

請按照以下格式生成回應：

🔮 小鈴看了你的八字，發現你有很特別的${enhancedResult.detectedTopic || "運勢"}潛質呢！✨
...
💖 哈囉親愛的[五行]命寶寶！小鈴來幫你分析${enhancedResult.detectedTopic || "運勢"}啦～根據你的五行特質，[年份]年會是[運勢特點]的一年呢！(´▽\`ʃ♡ƪ)
...
[五行]命寶寶記得要多用[幸運顏色]系物品增強運氣呢～小鈴祝你[祝福語]！٩(◕‿◕)۶
...
請直接給出解釋，不要說"我來解釋"之類的開場白。`;
					...
					// 🎂 先生成標準的服務引導回應（包含 AI 分析）
					const standardResponse =
						await classifier.generateServiceGuidance(
							analysis,
							message,
							sessionId
						);

					// 🔧 如果是感情類別且包含分手選項，保存 primaryConcern 以支持後續 A/B/C/D 選擇
					if (
						analysis.detectedTopic === "感情" &&
						(standardResponse.includes("A. 剛分手") ||
							standardResponse.includes("A. 刚分手"))
					) {
						console.log(
							"💔 檢測到分手流程，保存 primaryConcern = 感情"
						);
						if (!userIntent) {
							userIntent = new SmartUserIntent({
								sessionId: sessionId,
						...
						} else if (!userIntent.primaryConcern) {
							userIntent.primaryConcern = "感情";
						}
						await userIntent.save();
						console.log(
							"✅ 已保存 primaryConcern = 感情，支持分手選項處理, sessionId:",
							sessionId
						);
					}

					// 🎂 然後檢查是否有已保存的生日
					let birthdayCheckResult = null;
					if (
						analysis.isWithinScope &&
						analysis.detectedTopic !== "感情" &&
						analysis.detectedTopic !== "問候"
					) {
						console.log("🎂 檢查是否有已保存的生日...");

						// 提取 AI 分析部分（去除生日請求部分）
						let aiAnalysisOnly = standardResponse;
						// 移除生日請求的部分
						const birthdayRequestIndex =
							aiAnalysisOnly.indexOf("\n\n告訴小鈴你的生日");
						if (birthdayRequestIndex > 0) {
							aiAnalysisOnly = aiAnalysisOnly.substring(
								0,
								birthdayRequestIndex
							);
						}
						...
						birthdayCheckResult =
							await checkSavedBirthdayAndGenerateMessage(
								...
								userId,
								analysis.detectedTopic,
								aiAnalysisOnly, // 🎯 傳入 AI 分析內容
								locale // 🌐 傳入 locale 確保正確的語言
							);
					} // 如果有已保存的生日，使用包含 AI 分析的確認訊息；否則使用標準引導訊息
					if (
						birthdayCheckResult &&
						birthdayCheckResult.hasSavedBirthday
					) {
						console.log(
							"✅ 找到已保存的生日，使用包含 AI 分析的確認訊息"
						);
						response = birthdayCheckResult.message;
			...
			if (!userIntent) {
				...
				if (
					...
					if (userIntent.relationshipAnalysisType === "individual") {
						...
						userIntent.specificQuestion = "合盤配對分析";
					} else {
						userIntent.specificQuestion = `${userIntent.primaryConcern}諮詢`;
					}
				}
			} else if (analysis && analysis.isWithinScope) {
				...
				// 🎯 FIX: 不覆蓋已存在的 specificQuestion (例如用戶選擇輸入新生日時)
				// 只在以下情況更新 specificQuestion:
				// 1. specificQuestion 還未設置
				// 2. 或者是臨時狀態消息（如"等待輸入新生日"）且現在有更好的問題描述
				const shouldUpdateSpecificQuestion =
					!userIntent.specificQuestion ||
					userIntent.specificQuestion === "等待輸入新生日" ||
					userIntent.specificQuestion.includes("數字") ||
					userIntent.specificQuestion.includes("无法确定");

				if (shouldUpdateSpecificQuestion) {
		...
		if (userBirthday && gender && !message?.trim()) {
			...
			if (parsedDate) {
				...
				if (
					...
					reportUrl = `/zh-CN/couple-report?${params.toString()}`;
					console.log("💕 Smart-Chat2 生成合盤報告URL:", reportUrl);
				} else {
					// 個人分析：生成個人報告URL
					const params = new URLSearchParams({
						birthday: userBirthday,
	...
	try {
		...
		if (assistantResponse) {
			...
			if (
				...
				formattedResponse = formatCoupleAnalysisResponse(
					assistantResponse,
					locale
				);
			}

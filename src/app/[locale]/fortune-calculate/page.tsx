...
function getYearPillar(year: number): { stem: string; branch: string; element: string } {
...
function getDayPillar(date: Date): { stem: string; branch: string; element: string } {
...
function calculateElementScore(element1: string, element2: string): number {
  ...
  const relation = ELEMENT_RELATIONS[element1 as keyof typeof ELEMENT_RELATIONS];
...
function calculateDailyFortune(userBirthday: Date | null, selectedDate: Date): string {
  ...
  try {
    ...
    score += calculateElementScore(birthYearPillar.element, currentYearPillar.element) * 0.3;
    
    // Day interaction (daily fortune - most important)
    score += calculateElementScore(birthDayPillar.element, currentDayPillar.element) * 0.7;
    
    // Birth year vs current day (personal element meeting daily energy)
    score += calculateElementScore(birthYearPillar.element, currentDayPillar.element) * 0.5;
    
    // Birth day vs current year (daily essence in yearly context)
    score += calculateElementScore(birthDayPillar.element, currentYearPillar.element) * 0.3;
    ...
    if (opposingBranches[birthDayPillar.branch] === currentDayPillar.branch) {
      score -= 15;
    }
...
export default function FortuneCalculatePage() {
  ...
  const [dailyFortunes, setDailyFortunes] = useState<{[key: string]: string}>({});
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        const userEmail = session?.user?.email || mobileSession?.user?.email;
        ...
        if (Capacitor.isNativePlatform()) {
          const { value: savedBirthday } = await Preferences.get({ key: 'userBirthday' });
          if (savedBirthday) {
            try {
              setUserBirthday(new Date(savedBirthday));
              console.log('✅ Loaded birthday from mobile preferences:', savedBirthday);
            } catch (error) {
              console.error('❌ Invalid birthday format in preferences:', savedBirthday);
            }
          } else {
            console.warn('⚠️ No birthday in mobile preferences - user needs to set it in birthday-entry page');
          }
        }
        ...
        if (userEmail && typeof window !== 'undefined') {
          ...
          console.log('🌐 Fetching birthday from:', `${API_BASE}/api/user-profile`);
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (data.birthDateTime) {
                const apiBirthday = new Date(data.birthDateTime);
                setUserBirthday(apiBirthday);
                console.log('✅ Loaded birthday from API database:', data.birthDateTime);
                ...
                if (Capacitor.isNativePlatform()) {
                  ...
                  console.log('💾 Saved birthday to preferences for offline use');
                }
              } else {
                console.warn('⚠️ No birthDateTime in API response');
              }
            } else {
              console.log(`ℹ️ API not available (${response.status}) - using local data`);
            }
          } catch (error) {
            console.log('ℹ️ API fetch failed - using local data:', error.message);
          }
        } else if (!userEmail) {
          console.warn('⚠️ No user email found - cannot fetch birthday from API');
        }
        
        // Load saved fortunes from Capacitor Preferences (persistent on mobile)
        if (Capacitor.isNativePlatform()) {
          const { value: savedFortunes } = await Preferences.get({ key: 'dailyFortunes' });
          if (savedFortunes) {
            try {
              const parsed = JSON.parse(savedFortunes);
              setDailyFortunes(parsed);
              console.log('💾 Loaded cached fortunes from Preferences:', Object.keys(parsed).length, 'dates');
            } catch (error) {
              console.error('❌ Failed to parse saved fortunes:', error);
            }
          }
          ...
          if (savedFortunes) {
            ...
            console.log('💾 Loaded cached fortunes from localStorage');
          }
        }
      } catch (error) {
  ...
  const getFortuneForDate = async (date: Date): Promise<string> => {
    ...
    try {
      ...
      console.log('API call failed, using client-side calculation:', error);
      // Fallback to client-side calculation
      fortune = calculateDailyFortune(userBirthday, date);
    }
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      console.log('💾 Saved fortune to Preferences:', dateKey, '→', fortune);
    } else if (typeof window !== 'undefined') {
      // Fallback to localStorage for web
      localStorage.setItem('dailyFortunes', JSON.stringify(updatedFortunes));
      console.log('💾 Saved fortune to localStorage:', dateKey, '→', fortune);
    }
  ...
  const fortuneConfig = FORTUNE_LEVELS[currentFortune as keyof typeof FORTUNE_LEVELS] || FORTUNE_LEVELS['中吉'];
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      {!userBirthday && Capacitor.isNativePlatform() && (
        <div className="mx-4 mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800 mb-2">⚠️ 測試模式：未設定生日</p>
          <button
      ...
      <div className="mx-4 mt-4">
        ...
        >
          ...
          {!userBirthday && (
            <p className="text-sm mt-2 opacity-80">請在個人資料中設定生日以獲得準確運勢</p>
          )}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              {dateFortune && !isSelected && (
                <span className="text-[10px] mt-0.5 opacity-60">{dateFortune}</span>
              )}
            </button>
          );
        })}

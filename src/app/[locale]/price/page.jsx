...
export default function YourPage() {
	...
	const handleSqftSubmit = async () => {
		...
		try {
			...
			// 🔥 MOBILE FIX: Prepare headers with mobile session info
			const fetchHeaders = {
				"Content-Type": "application/json",
			};
	...
	const handleFortunePayment = async (concernType) => {
		...
		try {
			...
			// Prepare headers with mobile session support
			...
			if (isCapacitorMobile && effectiveSession?.user) {
				...
				fetchHeaders["X-User-ID"] =
					effectiveSession.user.userId || effectiveSession.user.id;
				console.log(
					"📱 Adding mobile session headers to fortune payment:",
					{
						email: fetchHeaders["X-User-Email"],
						userId: fetchHeaders["X-User-ID"],
					}
				);
			}

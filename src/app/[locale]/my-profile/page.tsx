      ...
      try {
        if (Capacitor.isNativePlatform()) {
          ...
          if (mobileSession?.user?.name) {
            setFormData(prev => ({ ...prev, nickname: mobileSession.user.name }));
          }

          // Load other data from Preferences
          const { value: gender } = await Preferences.get({ key: 'userGender' });
          const { value: birthday } = await Preferences.get({ key: 'userBirthday' });
          const { value: location } = await Preferences.get({ key: 'userBirthLocation' });
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="container mx-auto px-4 pt-20">
        ...
        <div className="text-center mb-6">
          <h1 className="text-xl font-bold text-gray-800 mb-2">修改個人資料</h1>
          <p className="text-sm text-gray-500">{new Date().toLocaleDateString('zh-TW')}</p>
        </div>
        ...
        <div className="mb-4">
          <h2 className="text-base font-medium text-gray-700">
            {mobileSession?.user?.name ? `${mobileSession.user.name} 的個人資料` : 'Thea 的個人資料'}
          </h2>
        </div>

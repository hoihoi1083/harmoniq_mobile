...
export default function Home() {
	...
	useEffect(() => {
		...
		if (pendingMessage && pendingTimestamp && effectiveSession?.user) {
	...
	const loadConversationHistory = async (userId) => {
		try {
			...
			const mobileEmail = mobileSession?.user?.email;

			console.log("🔍 Checking for session email:", {
				sessionEmail,
				mobileEmail,
			});
			...
			if (sessionEmail) {
				...
				// Use mobile session email if available
				queryParams.append("userEmail", mobileEmail);
				console.log(
					"📱 [MOBILE] Added mobile session email:",
					mobileEmail
				);
			} else {
	...
	return (
		<div className="bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50">
			{/* Navigation Bar - with iOS safe area support */}

...
filesToUpdate.forEach(filePath => {
  ...
  const hasAPIBase = content.includes('API_BASE') || content.includes('getApiBaseUrl');
  ...
  if (modified && !hasAPIBase) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('import ') || lines[i].includes('require(')) {
        insertIndex = i + 1;
      } else if (lines[i].includes('export default') || lines[i].includes('export const') || 
                 lines[i].includes('export function') || lines[i].includes('function ')) {

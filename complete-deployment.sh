...
# Configuration
REMOTE_HOST="fs"
REMOTE_PATH="/home/ec2-user/fengshui-mobile-backend"
LOCAL_PROJECT_PATH="."
...
server_build_and_deploy() {
    ...
    ssh $REMOTE_HOST << 'EOF'
        ...
        echo "🚀 Starting mobile backend deployment..."
        
        # Navigate to mobile backend directory
        cd /home/ec2-user/fengshui-mobile-backend
        
        # DON'T stop PM2 yet - keep site running during build
        echo "📦 Installing dependencies (site still running)..."
        npm install --production=false
        
        # Build the application (site still running)
        echo "🔨 Building application (site still running)..."
        npm run build
        ...
        # If reload fails (first deploy), start fresh
        if [ $? -ne 0 ]; then
